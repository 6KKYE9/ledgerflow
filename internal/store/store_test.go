...
func TestAddAndList(t *testing.T) {
	s := tempStore(t)
	s.Add(TypeIncome, 100, "工资", "月薪", nil, time.Now())
	s.Add(TypeExpense, 30, "餐饮", "午饭", nil, time.Now())
...
func TestFilter(t *testing.T) {
	s := tempStore(t)
	now := time.Now()
	s.Add(TypeExpense, 10, "餐饮", "面", nil, now)
	s.Add(TypeExpense, 20, "交通", "地铁", nil, now)
	s.Add(TypeIncome, 500, "工资", "", nil, now)
...
func TestUpdateDelete(t *testing.T) {
	s := tempStore(t)
	rec := s.Add(TypeExpense, 10, "餐饮", "面", nil, time.Now())
	if !s.Update(rec.ID, 15, "", "", nil, time.Time{}) {
		t.Fatal("Update 应成功")
	}
	got, _ := s.Get(rec.ID)
	if got.Amount != 15 {
...
func TestPersistence(t *testing.T) {
	dir := t.TempDir()
	s1, _ := New(dir)
	s1.Add(TypeIncome, 99, "工资", "", nil, time.Now())
